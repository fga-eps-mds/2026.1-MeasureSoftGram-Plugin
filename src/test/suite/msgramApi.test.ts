...
function createFullMockServer(): Promise<{ server: http.Server; baseUrl: string }> {
  return mockServer((req, res) => {
    const url = req.url ?? '';
    if (url === '/api/v1/organizations/')                                                          { return json(res, { count: 1, results: [MOCK_ORG] }); }
    if (url === `/api/v1/organizations/${MOCK_ORG.id}/products/`)                                 { return json(res, { count: 1, results: [MOCK_PRODUCT] }); }
    if (url === `/api/v1/organizations/${MOCK_ORG.id}/products/${MOCK_PRODUCT.id}/repositories/`) { return json(res, { count: MOCK_REPOS.length, results: MOCK_REPOS }); }
    if (url.includes('latest-values/tsqmi'))           { return json(res, url.includes('9999') ? { created_at: null } : MOCK_TSQMI); }
    if (url.includes('latest-values/characteristics')) { return json(res, { count: MOCK_CHARS.length, results: MOCK_CHARS }); }
...
suite('fetchRepositories', () => {
  ...
  test('lança erro quando produto não encontrado', async () => {
    const { server, baseUrl } = await mockServer((req, res) => {
      if (req.url === '/api/v1/organizations/') { return json(res, { count: 1, results: [MOCK_ORG] }); }
